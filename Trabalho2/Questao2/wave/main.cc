...
void iso_3dfd_it(float ***ptr_next, float ***ptr_prev, float ***ptr_vel, float *coeff, int n1, int n2, int n3){

	int block = 4;
  int ii,jj,kk,i,j,k,ir;
  float value;
  int half = HALF_LENGTH;
  //default(none) private(ii, jj, kk, i, j, k, value, ir) shared(ptr_next, ptr_prev, coeff, ptr_vel, n1, n2, n3, half, block)
#pragma omp parallel private(ii, jj, kk, i, j, k, value, ir) shared(ptr_next, ptr_prev, coeff, ptr_vel, n1, n2, n3, half, block)
  {
#pragma omp for schedule(runtime)
    for (ii = half; ii < n1 - half; ii += block){
      for (jj = half; jj < n2 - half; jj += block){
        for (kk = half; kk < n3 - half; kk += block){
          for (i = ii; i < std::min(n1 - half, ii + block); i++){
            for (j = jj; j < std::min(n2 - half, jj + block); j++){
              for (k = kk; k < std::min(n3 - half, kk + block); k++){
                value = 0.0;
